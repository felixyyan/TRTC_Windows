...
namespace tencentyun
{
    public class TLSSigAPIv2
    {
        ...
        private static byte[] CompressStream(Stream sourceStream)
        {
            using (MemoryStream streamOut = new MemoryStream())
            {
                using (ZlibStream streamZOut = new ZlibStream(streamOut, CompressionMode.Compress, CompressionLevel.Default))
                {
